...
#ifdef Q_OS_WIN
        // Team Edition: the control/teaching window must remain a regular,
        // resizable window. The geometry calculated above deliberately leaves
        // a margin around it, so do not override it with a full-screen call.
        controlWidget->setWindowState(controlWidget->windowState()
                                      & ~Qt::WindowFullScreen
                                      & ~Qt::WindowMaximized);
        controlWidget->showNormal();
#else

...
#ifdef Q_OS_WIN
    // Team Edition always starts with a real Windows frame. This must be set
    // before UBMainWindow is constructed; calling showNormal() later cannot
    // restore a title bar to a window created with FramelessWindowHint.
    UBSettings::settings()->appRunInWindow->set(true);
#endif

    ...
    */
    ...
    if (mShouldRecordAudio) {
        ...
        avcodec_get_supported_config(c, audioCodec, AV_CODEC_CONFIG_SAMPLE_FORMAT,
                                     0, &supportedSampleFormats, nullptr);
        const AVSampleFormat* sampleFormats = static_cast<const AVSampleFormat*>(supportedSampleFormats);
        c->sample_fmt = sampleFormats ? sampleFormats[0] : AV_SAMPLE_FMT_FLTP;
#else

...
bool UBWindowsMediaVideoEncoder::start()
{
    ...
    mTemporaryWmvFileName = videoFileName();
    if (mTemporaryWmvFileName.endsWith(".mp4", Qt::CaseInsensitive))
        mTemporaryWmvFileName.chop(4);
    mTemporaryWmvFileName += ".recording.wmv";

    if(!mWMVideo->init(QString(mTemporaryWmvFileName).replace("/", "\\"), profile, framesPerSecond(),
...
bool UBWindowsMediaVideoEncoder::stop()
{
    ...
    if (ok)
    {
        emit encodingStatus(tr("Creating MP4 video ..."));

        const QString ffmpegPath = QCoreApplication::applicationDirPath() + "/ffmpeg.exe";
        if (!QFileInfo::exists(ffmpegPath))
        {
            mLastErrorMessage = tr("FFmpeg was not found. The recording was preserved as %1")
                    .arg(mTemporaryWmvFileName);
            ok = false;
        }
        else
        {
            ...
            arguments << "-y" << "-i" << mTemporaryWmvFileName
                      << "-c:v" << "libx264" << "-preset" << "medium"
                      << "-crf" << "22" << "-pix_fmt" << "yuv420p"
                      << "-c:a" << "aac" << "-b:a" << "128k"
                      << "-movflags" << "+faststart" << videoFileName();

            ffmpeg.start(ffmpegPath, arguments);
            const bool started = ffmpeg.waitForStarted(10000);
            const bool finished = started && ffmpeg.waitForFinished(-1);
            if (!started || !finished || ffmpeg.exitStatus() != QProcess::NormalExit
                    || ffmpeg.exitCode() != 0)
            {
                mLastErrorMessage = tr("Could not create MP4. The recording was preserved as %1. %2")
                        .arg(mTemporaryWmvFileName,
                             QString::fromLocal8Bit(ffmpeg.readAllStandardError()).right(1000));
                QFile::remove(videoFileName());
                ok = false;
            }
            else
            {
                QFile::remove(mTemporaryWmvFileName);
            }
        }
    }

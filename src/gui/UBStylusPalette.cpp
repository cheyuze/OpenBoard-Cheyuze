    ...
    , mLastSelectedId(-1)
{
    ...
    shapesAction->setToolTip(tr("Common shapes: circles, squares, triangles and arrows"));
    shapesAction->setProperty("ungrouped", true);
    connect(shapesAction, &QAction::triggered, this, []() {
        UBBoardPaletteManager* paletteManager = UBApplication::boardController->paletteManager();
        paletteManager->featuresWidget()->showShapes();
        paletteManager->rightPalette()->activateWidget(paletteManager->featuresWidget());
    });
    actions << shapesAction;
